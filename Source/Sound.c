...
void PlaySound1 (short soundID, short priority)
{
	...
	theErr = SndDoImmediate(externalChannel, &theCommand); /* TODO: read value stored to theErr somewhere... */
	...
	theErr = SndDoCommand(externalChannel, &theCommand, TRUE); /* TODO: read value stored to theErr somewhere... */
}
...
void PlaySound2 (short soundID, short priority)
{
	...
	theCommand.param1 = 0;
	theCommand.param2 = 0L;
	theErr = SndDoImmediate(externalChannel2, &theCommand); /* TODO: read value stored to theErr somewhere... */
	...
	theErr = SndDoCommand(externalChannel2, &theCommand, TRUE); /* TODO: read value stored to theErr somewhere... */
}
...
pascal void ExternalCallBack (SndChannelPtr theChannel, SndCommand *theCommand)
{
	...
	if (theCommand->param1 == kSoundDone)
	{
		...
		thisA5 = SetA5(thisA5); /* TODO: read value stored to thisA5 somewhere... */
	}
}
...
OSErr LoadBufferSounds (void)
{
	...
	{
		theSound = GetResource('snd ', i + kBaseBufferSoundID); /* FIXME: multi-character constant? */
		if (theSound == 0L)
			return (ResError());
...
void KillSound (void)
{
	...
	theErr = DumpBufferSounds(); /* TODO: read value stored to theErr somewhere... */
	theErr = CloseSoundChannel(); /* TODO: read value stored to theErr somewhere... */
}
