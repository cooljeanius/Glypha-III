...
AC_ARG_WITH([gcc-arch],
            [AS_HELP_STRING([--with-gcc-arch=<arch>],
                            [use architecture <arch> for gcc -march/-mtune, instead of guessing])],
...
for arch in ${ax_gcc_arch}; do
  if test "x[]m4_default([$1],[yes])" = "xyes"; then # if we require portable code
    flags="-mtune=${arch}"
    # -mcpu=${arch} and m${arch} generate nonportable code on each arch but
    # x86. And some other arches (e.g. Alpha) do NOT accept -mtune. Grrr.
