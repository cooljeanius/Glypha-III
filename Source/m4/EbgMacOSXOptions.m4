...
AC_DEFUN([AC_GET_MACOSX_VERSION],[
    ...
    case $macosx_version in
         ...
         *10.4*)
         ...
         ;;
         *)
              AC_MSG_WARN([While it might be better to build this on an older version of OS X, such as your $macosx_version, as fewer things are deprecated there, it's still harder to find computers running older versions of OS X these days, so it might be harder to support...])
         ;;
	 esac
])
