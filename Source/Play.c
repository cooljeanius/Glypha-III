...
void ResetPlayer(Boolean initialPlace)
{
	...
	switch (location) {
		case 0:
			thePlayer.h = (296 << 4); /* bottom center ((296 << 4) = 4736) */
			thePlayer.v = (377 << 4); /* ((377 << 4) = 6032) */
			break;

		case 1:
			thePlayer.h = (102 << 4); /* ((102 << 4) = 1632) */
			thePlayer.v = (237 << 4); /* ((237 << 4) = 3792) */
			break;
...
#define kTicksPerFrame 2L
	...
	waitUntil = (long)(TickCount() + kTicksPerFrame);
						...
						switch (thePart) {
							case inMenuBar:
								...
								thePart = FindWindow(theEvent.where,
													 &whichWindow);
								/* dummy condition to use value stored to
								 * 'thePart': */
								if (thePart == (WindowPartCode)0) {
									;
								}
